...
export function registerSalesRuntimeTools(): void {
  // Replace Gemini's mock/fabricated WEB_SCRAPE implementation with real public-web fetching.
  ...
  toolRegistry.registerTool({
    ...
    execute: async (args: any) => crawlPublicBusinessSite({
      url: String(args.url),
  ...
  toolRegistry.registerTool({
    ...
    validateInput: (args: any) => {
      if (!args || !Number.isFinite(Number(args.maximumResults || 10))) {
        return { valid: false, error: 'valid discovery arguments are required' };
      }
      return { valid: true };
    },
    execute: async (args: any) => {
      ...
      if (!provider.isConfigured()) {
        return { status: 'configuration_required', provider: provider.name, prospects: [] };
      }
  ...
  toolRegistry.registerTool({
    ...
    retryBehavior: { maxAttempts: 1, backoffMs: 0 },
    validateInput: (args: any) => {
      if (!args?.prospect?.website || !args?.prospect?.domain) {
        return { valid: false, error: 'prospect website and domain are required' };
      }
      if (!Array.isArray(args.targetIndustries) || !Array.isArray(args.targetLocations)) {
        return { valid: false, error: 'targetIndustries and targetLocations are required' };
      }
      return { valid: true };
    },
